...
- `origin` → `anonymous-submission-repo` (개인 리포지토리)
- `anon` → `anonymous-submission-repo` (익명 제출용 리포지토리)

**모든 push는 반드시 양쪽에 동시 수행해야 합니다.**

## 환경 정보 (MiKTeX on Windows)

- **TeX**: `pdflatex` via MiKTeX on `PATH`
- **latexmk 사용 불가**: MiKTeX에 perl 미설치 → `pdflatex` 직접 3-pass 사용
- **bibtex**: 사용 가능
- **Python**: conda base, repo root 기준
...
```powershell
python NeurIPS2026_final_submission\code\scripts\package_supplementary.py
```
...
- 개인 경로/이름 익명화 (실명, 로컬 계정명, 비익명 경로 제거)
