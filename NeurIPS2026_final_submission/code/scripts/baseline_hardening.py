...
- HP sweep expanded: 5 lr × 5 entropy × 5 gamma = 125 per paradigm → 500+ total
...
# Expanded HP Grid (5 × 5 × 4 = 100 per paradigm, × 4 paradigms ≈ 400+)
...
class LinearPolicy:
    """Simple linear policy: λ = sigmoid(w · obs + b)"""
...
class TabularQ:
    ...
    def act(self, obs, rng):
        ...
        return a / (self.n_actions - 1), a  # λ, action_idx
...
def main():
    ...
    print(f"\n  Paradigms: {paradigms}")
    print(f"  HP Grid: {len(LR_GRID)} lr × {len(ENTROPY_GRID)} ent × {len(GAMMA_GRID)} γ = "
            ...
            for ent in ENTROPY_GRID:
                for gam in GAMMA_GRID:
                    ...
                    print(f"  [{config_idx:3d}/{total_configs}] {paradigm} {key}: "
                          f"λ={np.mean(lams):.3f} surv={np.mean(survs):.0f}% [{status}]")
