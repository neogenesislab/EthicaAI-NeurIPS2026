...
IQL$^\ddagger$~\citep{rashid2018qmix} & 5,195 & 0.584 $\pm$ 0.021 & 71.8 & 24.9 \\
\midrule
...
\multicolumn{5}{l}{\small $^\dagger$Plus shared critic. $^\ddagger$IQL = Independent Q-Learning with shared team reward (no mixing network). 2-layer MLP (64 hidden), Adam.} \\
\end{tabular}}
...
Standard MARL implementations (CleanRL-style) spanning three paradigms---independent policy gradient (IPPO), centralized critic (MAPPO), and independent Q-learning (IQL)---all converge to suboptimal equilibria.
...
IPPO converges to $\lambda = 0.412 \pm 0.080$ (38.7\% survival, 20 seeds), and MAPPO to $\lambda = 0.392 \pm 0.070$ (37.0\%).
Notably, IQL achieves significantly higher cooperation ($\lambda = 0.584 \pm 0.021$, 71.8\% survival). We attribute this gap to the discrete action space and $\varepsilon$-greedy exploration of Q-learning, which partially circumvents the local optima that policy gradient methods converge to. However, even IQL falls \emph{substantially} short of the oracle ($\lambda = 1.0$, 100\% survival), confirming that the Nash Trap remains a robust structural barrier regardless of the learning paradigm.
...
\begin{itemize}
    ...
    \item \textbf{Baseline Scope}: The value-based baseline labeled ``IQL'' uses independent per-agent Q-learning with shared team reward, \textbf{without} the monotonic mixing network of QMIX~\citep{rashid2018qmix}. Consequently, IQL's higher $\lambda$ may reflect action-space discretization and $\varepsilon$-greedy exploration rather than centralized value decomposition. A full QMIX implementation with mixing network remains future work.
