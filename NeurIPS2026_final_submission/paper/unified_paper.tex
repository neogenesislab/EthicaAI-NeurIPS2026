...
Standard MARL implementations (CleanRL-style) spanning three paradigms---independent policy gradient (IPPO), centralized critic (MAPPO), and value decomposition (QMIX)---with \emph{unmodified default hyperparameters} all confirm the Nash Trap.
...
This result holds at $N=100$ (Table~\ref{tab:scale}), under K-level opponent anticipation ablations (Appendix~\ref{app:kpg}), across a hyperparameter sweep (20 combinations $\times$ 10 seeds = 200 runs, Appendix~\ref{app:hp_sweep}), and under partial observability and adaptive adversaries (Appendix~\ref{app:robustness}).
