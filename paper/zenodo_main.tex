...
(1) Dynamic meta-ranking significantly enhances collective welfare ($p{<}0.001$ via LMM with cluster bootstrap SE), while static SVO injection yields no welfare improvement.
...
(3) \textbf{Situational Commitment} achieves utilitarian-optimal welfare (147.7 per-step) using only local information, while Utilitarian strategy dominates evolutionary competition (99.6\% ESS vs.\ 0.1\% for Situational).
...
\begin{table}[h]
\centering
\caption{SOTA comparison on PGG ($N{=}50$, 10 seeds, per-step welfare). Byz-50\% = cooperation under 50\% adversaries. $\dagger$ = requires opponent parameter access. LOLA cooperation (0.209) reflects 45-hour full-sweep analytical approximation; IPPO baseline trained identically to Meta-Ranking.}
\label{tab:sota}
\begin{tabular}{lccccc}
\toprule
Algorithm & Coop & Welfare & Gini ($\downarrow$) & Byz-50\% & Decentral. \\
\midrule
IPPO Baseline & 0.518 & 108.3 & 0.011 & 0.201 & \checkmark \\
Inequity Aversion & 0.414 & 124.8 & 0.006 & 0.025 & \checkmark \\
Social Influence & 0.927 & 155.6 & 0.000 & 0.145 & \checkmark \\
LOLA$^\dagger$ & 0.209 & 101.4 & 0.014 & 0.093 & \texttimes \\
\textbf{Meta-Ranking (Ours)} & 0.859 & 147.7 & 0.008 & \textbf{0.369} & \checkmark \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textit{Note on welfare scale}: Table~\ref{tab:main} reports \textbf{cumulative} welfare ATE over 200 PGG rounds (Dynamic vs.\ Static $\lambda$), while Tables~\ref{tab:moral}--\ref{tab:sota} report \textbf{per-step mean} welfare within a single condition. The two scales are not directly comparable; ATE captures the \textit{treatment effect magnitude} across full episodes.
