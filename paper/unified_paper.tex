...
\bottomrule
\multicolumn{5}{l}{\small Standard implementations with default hyperparameters; see Appendix~\ref{app:baselines}.} \\
...
While QMIX's value decomposition yields higher cooperation than policy gradient methods, it still falls \emph{substantially} short of the oracle ($\lambda = 1.0$, 100\% survival), confirming that the Nash Trap is a fundamental property of the environment rather than an artifact of any particular algorithmic paradigm.
This result holds at $N=100$ (Table~\ref{tab:scale}), under K-level opponent anticipation ablations (Appendix~\ref{app:kpg}), and across a hyperparameter sweep of learning rate and entropy coefficient (Appendix~\ref{app:hp_sweep}).
...
\begin{itemize}
    \item \textbf{Algorithm Scope}: Six algorithms across three paradigms all converge to the Nash Trap, confirming it is a game-theoretic property. Whether cooperative intrinsic motivation~\citep{jaques2019social} or dynamic externality-pricing mechanisms (e.g., Pigovian tax-based reward shaping) can escape the Trap \emph{without encoding cooperation} remains open.
    \item \textbf{Environment Scope}: Primary results use PGG with scalar $R_t$. CPR validation (Appendix~\ref{app:cpr}) shows the same pattern (selfish 9.3\%, situational 6.5\%, unconditional 46.0\%). Melting Pot~\citep{leibo2021scalable} benchmarking remains future work.
    \item \textbf{Price Equation}: Group-level ESS requires $m < m^* = 0.286$ (Result~\ref{res:group_ess}). Sensitivity analysis (Appendix~\ref{app:sensitivity}) over $M \in [1.2,2.0]$ confirms Nash Trap persistence ($\lambda$ range 0.43--0.52).
\end{itemize}
