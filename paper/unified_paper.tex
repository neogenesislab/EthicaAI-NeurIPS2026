...
\bottomrule
\multicolumn{5}{l}{\small Standard CleanRL implementations with default hyperparameters; see Appendix~\ref{app:baselines}.} \\
\multicolumn{5}{l}{\small $^\dagger$Plus shared critic. 2-layer MLP (64 hidden), Adam, GAE($\gamma{=}0.99$, $\tau{=}0.95$).} \\
\end{tabular}
\footnotetext{All algorithms are statistically significantly below the oracle ($p < 0.001$, Mann-Whitney $U$ test, 20 seeds). Pairwise: IPPO vs.\ MAPPO survival difference is \emph{not} significant ($p > 0.05$). Bootstrap 95\% CIs: IPPO $\lambda$ [0.382, 0.438], survival [33.0, 42.0]\%; MAPPO $\lambda$ [0.369, 0.417], survival [32.3, 41.7]\%. Wall-clock: $\sim$45s/seed (CPU, 300 episodes).}
\end{table}
...
Standard MARL implementations using CleanRL~\citep{huang2022cleanrl} with \emph{unmodified default hyperparameters} confirm the Nash Trap: IPPO converges to $\lambda = 0.409 \pm 0.064$ (37.5\% survival, 20 seeds) and MAPPO with shared critic to $\lambda = 0.394 \pm 0.054$ (37.0\% survival).
Algorithms with richer information sharing do not improve survival ($p > 0.05$ for IPPO vs.\ MAPPO, Mann-Whitney $U$ test); the shared critic does not overcome the fundamental $M/N = 0.08 < 1$ free-riding incentive.
...
We directly adapt the reference CleanRL~\citep{huang2022cleanrl} implementations---the most widely-cited single-file MARL codebase---with \emph{zero hyperparameter tuning}:
\begin{itemize}\setlength{\itemsep}{0pt}
...
\noindent Both IPPO ($\lambda = 0.409$, 95\% CI $[0.382, 0.438]$) and MAPPO ($\lambda = 0.394$, 95\% CI $[0.369, 0.417]$) converge squarely into the Nash Trap region, confirming that the cooperation failure is not an artifact of our minimal implementations but a fundamental property of the non-linear PGG environment under standard MARL training.
...
\item \textbf{Experiments}:
  \begin{enumerate}
    ...
    \item Seeds: 5--20 per experiment, specified per table caption. [Yes]
  \end{enumerate}
