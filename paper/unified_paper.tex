...
\end{tabular}
\footnotetext{All algorithms are statistically significantly below the oracle ($p < 0.001$, Mann-Whitney $U$ test, 20 seeds). Bootstrap 95\% CIs: IPPO $\lambda$ [0.382, 0.438], surv. [33.0, 42.0]\%; MAPPO $\lambda$ [0.369, 0.417], surv. [32.3, 41.7]\%; QMIX $\lambda$ [0.551, 0.568], surv. [63.8, 71.3]\%. QMIX achieves higher $\lambda$ but still falls substantially short of the oracle. Wall-clock: $\sim$19--45s/seed (CPU).}
\end{table}
...
We implement QMIX~\citep{rashid2018qmix} with per-agent Q-networks (same 2-layer MLP architecture, 5,195 params/agent) and a monotonic mixing network that aggregates individual Q-values into $Q_{\text{tot}}$ via hypernetwork-generated positive weights:
\begin{itemize}\setlength{\itemsep}{0pt}
...
\noindent All three paradigms---independent policy gradient (IPPO, $\lambda = 0.409$), centralized critic (MAPPO, $\lambda = 0.394$), and value decomposition (QMIX, $\lambda = 0.560$)---converge below the oracle, confirming that the Nash Trap is a fundamental property of the non-linear PGG environment.
