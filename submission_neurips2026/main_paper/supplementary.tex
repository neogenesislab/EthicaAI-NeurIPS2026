...
\noindent\textbf{Static vs Dynamic (G3).} Static $\lambda = \sin\theta$ fails to improve welfare compared to baseline, confirming that the \textit{dynamic} component is essential, not merely the moral weighting (Fig. 5--8).
...
All core experiments from the main paper are reproduced via a single script:

\begin{lstlisting}[language=bash]
pip install numpy matplotlib  # Only dependencies
python scripts/reproduce_quick.py  # ~2 min, CPU only
\end{lstlisting}

The \texttt{reproduce\_quick.py} script runs 5 core experiments sequentially (Nash Trap verification, DNN ablation, commitment sweep, scale test $N{=}100$, evolutionary stability) and outputs results to \texttt{outputs/quick\_verify/}. Each experiment uses fixed random seeds for deterministic reproduction. Total runtime: approximately 2 minutes on any modern CPU (no GPU required).
