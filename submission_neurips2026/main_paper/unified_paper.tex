...
We demonstrate that this failure is \textbf{algorithm-invariant}: independent policy-gradient agents converge to suboptimal equilibria regardless of architecture---linear REINFORCE ($\lambda \approx 0.49$), MLP ($\lambda \approx 0.05$, extreme free-riding), and MLP Actor-Critic ($\lambda \approx 0.05$)---all achieving $\leq$6\% survival under 30\% Byzantine adversaries.
...
\begin{enumerate}
    ...
    \item[(C3)] \textbf{Algorithm-Invariant Cooperation Failure}: When tipping points introduce irreversible collapse, independent RL agents fail regardless of architecture (Linear, MLP, Actor-Critic)---converging to suboptimal equilibria ($\lambda \approx 0.05$--$0.5$) with $\leq$6\% survival (Section~\ref{sec:nash_trap}).
...
\midrule
IPPO Linear & 5 & 0.488 $\pm$ 0.007 & 28.8 & 25.3 \\
IPPO MLP & 161 & 0.048 $\pm$ 0.004 & \textbf{6.0} & 20.5 \\
IPPO MLP + Critic & 322 & 0.048 $\pm$ 0.004 & \textbf{6.0} & 20.5 \\
\midrule
...
\textbf{Result}: Cooperation failure is \textbf{algorithm-invariant} (Table~\ref{tab:emergence}). Linear agents converge to $\lambda \approx 0.49$ (the ``Nash Trap''), while \emph{stronger} MLP and Actor-Critic agents converge to $\lambda \approx 0.05$---even more extreme free-riding.
...
All architectures achieve $\leq$6\% group survival.
