...
\begin{table}[t]
\centering
\caption{Cooperation failure across evaluated algorithms: all tested approaches converge to suboptimal equilibria in the non-linear PGG ($N{=}20$, Byz=30\%, $T$=50 rounds, $R_{\text{crit}}$=0.15, last 30 episodes, 5--10 seeds).}
\label{tab:emergence}
\begin{tabular}{lcccc}
\toprule
...
Notably, algorithms with richer information sharing achieve \emph{worse} survival in our implementation: the shared critic and centralized Q enable agents to \emph{more precisely} identify the free-riding incentive ($M/N = 0.08 < 1$).
